...
# LINK TO GALAXY
def link_to_galaxy(args:list, logger=logging.getLogger(__name__)) -> None:
    from .link_to_galaxy import link_to_galaxy
    link_to_galaxy(args.input_file, logger=logger, galaxy=args.galaxy, user=args.user,
...
link_parser.add_argument('-i', '--input_file',
        ...
        help='''Full or relative input file path to the existing TOMO workflow representations as 
                a NeXus file.''')
link_parser.add_argument('-l', '--log',
        choices=logging._nameToLevel.keys(),
        default='INFO',
        help='''Specify a preferred logging level.''')
